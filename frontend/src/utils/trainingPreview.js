...
function sumDistinctDistances(text) {
    let totalKm = 0;
...
function detectTags(text) {
    ...
    if (/\btempo\b/.test(normalized))
        tags.push("tempo");
    if (/\bfartlek\b/.test(normalized))
        tags.push("fartlek");
    if (/\b(kopec|kopce)\b/.test(normalized))
        tags.push("hill");
    if (/\bklus\b/.test(normalized))
        tags.push("easy");
    if (/\bvolno|rest\b/.test(normalized))
        tags.push("rest");
    if (/\binterval\b/.test(normalized))
        tags.push("interval");
    return tags;
}
...
function estimateTotalKm(text, intervals) {
    const normalized = text.trim().toLowerCase();
    if (!normalized) {
        return { totalKm: 0, confidence: "low", warning: "" };
    }
    if (["volno", "rest", "rest day"].includes(normalized)) {
        return { totalKm: 0, confidence: "high", warning: "" };
    }
    ...
    if (heuristicKm > 0) {
        return {
            totalKm: heuristicKm,
            confidence: heuristicKm >= 2 ? "medium" : "low",
            warning: klusMinutes > 0 || pauseMinutes > 0
...
export function parseTrainingPreview(text) {
