...
class DashboardFrontendRegressionTests(SimpleTestCase):
    ...
    def test_base_template_uses_shared_partials_and_static_ui_assets(self):
        ...
        self.assertIn('data-bs-target="#coachLegendModal"', top_nav_html)
        self.assertIn('data-bs-target="#importActivitiesModal"', top_nav_html)
        self.assertIn("eb-top-nav-icon-btn--legend", top_nav_html)
        self.assertIn("eb-top-nav-icon-btn--import", top_nav_html)
        base_css = BASE_CSS_PATH.read_text(encoding="utf-8")
        self.assertIn(".eb-top-nav-icon-btn--import {", base_css)
        self.assertIn(".eb-profile-modal .modal-dialog {", base_css)
        self.assertIn("max-height: calc(100vh - 16px);", base_css)
        self.assertIn(".eb-profile-avatar {", base_css)
        self.assertIn(".eb-profile-form-sm .form-control {", base_css)
        self.assertIn("window.EB.ui.setButtonBusy", base_js)
        self.assertIn("window.EB.notifications", base_js)

    def test_dashboard_header_partial_is_no_longer_included_on_athlete_dashboard(self):
        dashboard_html = DASHBOARD_TEMPLATE_PATH.read_text(encoding="utf-8")
        self.assertNotIn('{% include "dashboard/_dashboard_header.html" %}', dashboard_html)
    ...
    def test_coach_template_uses_split_sidebar_toolbar_and_modals(self):
        ...
        self.assertIn('eb-sidebar-current-athlete-card', coach_sidebar_html)
        self.assertIn('eb-sidebar-current-athlete-row', coach_sidebar_html)
        self.assertIn('eb-sidebar-legend-trigger', coach_sidebar_html)
        self.assertIn('id="coachAthletesToggleBtnMain"', coach_toolbar_html)
        self.assertIn('eb-top-nav-icon-btn--legend', coach_sidebar_html)
    ...
    def test_coach_mobile_responsive_rules_keep_sidebar_and_toolbar_usable(self):
        ...
        self.assertIn("is-mobile-athlete-selection-required", css)
        self.assertIn("is-mobile-athlete-picker-open", css)
        self.assertIn("is-mobile-plan-visible", css)
        self.assertIn("body.eb-plan-page {", css)
        self.assertIn("overflow: hidden;", css)
        self.assertIn(".eb-coach-main {", css)
        self.assertIn("max-height: 100%;", css)
        self.assertIn("overflow-x: auto;", css)
        self.assertIn("position: absolute;", css)
        self.assertIn("overflow-y: auto;", css)
        self.assertIn("overscroll-behavior: contain;", css)
        self.assertIn("#coachAthleteFocusInput", css)
        self.assertIn("#importActivitiesModal .modal-dialog", css)
        self.assertIn("#coachRemoveAthleteModal .modal-dialog", css)
        self.assertIn("#ebPlannedKmRulesModal .modal-dialog", css)
        self.assertIn(".eb-month-switcher--embedded {", css)
        self.assertIn("position: sticky;", css)
        self.assertIn(".eb-month-switcher--embedded .container-fluid", css)
        self.assertIn(".eb-month-switcher--embedded .eb-month-switcher-track", css)
        self.assertIn(".eb-sidebar-current-athlete-card", css)
        self.assertIn(".eb-sidebar-current-athlete-row", css)
        self.assertIn(".eb-manage-menu {", css)
        self.assertIn(".eb-manage-athlete-row .btn {", css)
    ...
    def test_coach_js_contains_mobile_athlete_picker_flow(self):
        coach_js = JS_COACH_PATH.read_text(encoding="utf-8")

        self.assertIn("eb_coach_mobile_show_plan_once", coach_js)
        self.assertIn("is-mobile-athlete-selection-required", coach_js)
        self.assertIn("is-mobile-athlete-picker-open", coach_js)
        self.assertIn("is-mobile-plan-visible", coach_js)
        self.assertIn('querySelector(".eb-coach-sidebar")', coach_js)
        self.assertIn("currentSidebar.replaceWith(replacementSidebar);", coach_js)
        self.assertIn('closest(".eb-athlete-switch-item[href]")', coach_js)
        self.assertIn("_coachRevealPlanAfterSelection", coach_js)
